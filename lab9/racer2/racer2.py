...
class Enemy(pygame.sprite.Sprite): 
    ...
    def move(self): 
        global score
        self.rect.move_ip(0, self.speed) #moves the enemy down 5 pixels 
...
silver_image = pygame.image.load('silver.png')
...
class Coins(pygame.sprite.Sprite): 
    def __init__(self, image): 
        ...
        self.image = image
        self.image_tr = pygame.transform.scale(image, (30, 30))
        self.rect = self.image_tr.get_rect() 
...
coins = pygame.sprite.GroupSingle() #sprite group of objects 
coins.add(Coins(silver_image))
coins_im = [silver_image, gold_image]
...
while True: 
    ...
    #moves and re draws all sprites: Player and Enemy
    for entity in all_sprites:
       entity.move()
       display.blit(entity.image, entity.rect) 

    #draws that one coin on its pos each new frame, move is kinda useless, since it is predetermined that its location is out of the scale of the enemy etc. 
    for ent in coins: 
        ent.move() 
        display.blit(ent.image_tr, ent.rect)
    ...
    if pygame.sprite.spritecollideany(P1,coins): 
        if coins.sprite.image == silver_image: coins_score +=1 
        elif coins.sprite.image == gold_image: coins_score +=3
        if coins_score% 5 == 0: E1.speed += 2
        #since the sprite group is single, when the new sprite is added the prev is deleted
        c_im = random.choice(coins_im) #randomly choose the image for the object to be inserted
        coins.add(Coins(c_im))
